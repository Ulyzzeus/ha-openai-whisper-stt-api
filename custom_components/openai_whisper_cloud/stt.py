...
async def async_setup_entry(
    ...
    async_add_entities([
        OpenAIWhisperCloudEntity(
            ...
            api_url=config_entry.data[CONF_URL] if config_entry.data.get(CONF_CUSTOM_PROVIDER) else provider.url,
            api_key=config_entry.data.get(CONF_API_KEY, ""),
            model= WhisperModel(config_entry.options[CONF_MODEL], SUPPORTED_LANGUAGES) if config_entry.data.get(CONF_CUSTOM_PROVIDER) else provider.models[config_entry.options[CONF_MODEL]],
...
class OpenAIWhisperCloudEntity(SpeechToTextEntity):
    ...
    def __init__(self, hass: HomeAssistant, custom: bool, provider_name: str, api_url: str, api_key: str, model: WhisperModel, temperature, prompt, name, unique_id) -> None:
        """Init STT service."""
        self.hass = hass
        self.custom = custom
    ...
    async def async_process_audio_stream(
        ...
        async for chunk in stream:
            audio_data += chunk
            if len(audio_data) / (1024 * 1024) > 24.5:
        ...
        try:
            ...
            if self.provider_name == "Deepgram":
                url = f"{self.api_url}?model={self.model.name}&detect_language=true"
                headers = {"Authorization": f"Token {self.api_key}", "Content-Type": "audio/wav"}
                response = await asyncio.to_thread(requests.post, url, headers=headers, data=temp_file.read())
            else:
                files = { "file": ("audio.wav", temp_file, "audio/wav") }
                data = { "model": self.model.name, "language": metadata.language, "temperature": self.temperature, "prompt": self.prompt, "response_format": "json" }
                response = await asyncio.to_thread(requests.post, f"{self.api_url}/v1/audio/transcriptions" if not self.custom else self.api_url, headers={ "Authorization": f"Bearer {self.api_key}" }, files=files, data=data)
            ...
            _LOGGER.debug("Full Deepgram Response: %s", response_json)
            ...
            if self.provider_name == "Deepgram":
                ...
                if detected_language:
                    self.hass.bus.async_fire(f"{DOMAIN}_language_detected", {"language": detected_language})
                    _LOGGER.debug(f"Fired event with detected language: {detected_language}")
            ...
            _LOGGER.error(f"Could not parse API response: {e}")
            if response is not None: _LOGGER.error(response.text)
            return SpeechResult("", SpeechResultState.ERROR)
